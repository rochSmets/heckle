...
typedef struct GhostG4Current_s
{
...
/*---------------------------------------------------------------------------
 ...
 ---------------------------------------------------------------------------*/
static void findGhostIndices(GhostG4Current *self, const STX* const sx)
{
...
{
    int t;
    GhostG4Current* self = malloc(sizeof *self);
    ...
    MPI_Get_address(&gptmp.jx, &tmpaddress);
    displacements[0] = tmpaddress - startaddress;

    MPI_Get_address(&gptmp.jy, &tmpaddress);
    displacements[1] = tmpaddress - startaddress;

    MPI_Get_address(&gptmp.jz, &tmpaddress);
        ...
        for (i = is0; i <= is1; i++)
        {
            for (j = js0; j <= js1; j++)
            {
                for (k = ks0; k <= ks1; k++)
                {
                    ...
                    self->sendbuff[t][cpt].jx = s4[ijk].j_smoothed[0];
                    self->sendbuff[t][cpt].jy = s4[ijk].j_smoothed[1];
                    self->sendbuff[t][cpt].jz = s4[ijk].j_smoothed[2];
            ...
            for (i = ir0; i <= ir1; i++)
            {
                for (j = jr0; j <= jr1; j++)
                {
                    for (k = kr0; k <= kr1; k++)
                    {
                        ...
                        s4[ijk].j_smoothed[0] = self->recvbuff[t][cpt].jx;
                        s4[ijk].j_smoothed[1] = self->recvbuff[t][cpt].jy;
                        s4[ijk].j_smoothed[2] = self->recvbuff[t][cpt].jz;
