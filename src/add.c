...
void add(struct sti *si, struct stx *sx, struct st2 *s2, struct stp *sp[NS+1], MPI_Comm com)
{
double ww;
double nb;
double aw[3], ew[3];
...
for (s = 1; s < NS+1; s++)
    ...
    for (j = 0; j < sx->n[1]; j++)
        ...
        for (k = 0; k < sx->n[2]; k++)
            ...
            nb = modelDensity(*si, *sx, st, qw, s);

            /* __ needed for the # of part to add __ */
            ww = (nb - s2[ijk].ns[s])/si->ws[s];
    ...
    MPI_Allreduce(&(sx->ns[s]), &(sx->nm[s]), 1, MPI_LONG_LONG_INT, MPI_MAX, co);
